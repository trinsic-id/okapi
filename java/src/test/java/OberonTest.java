...
class OberonTest {
  ...
  void testOberonDemo() throws InvalidProtocolBufferException, DidException {
    ...
    var token =
        Oberon.createToken(
            Security.CreateOberonTokenRequest.newBuilder()
                .setData(data)
                .setSk(key.getSk())
                .build());
    var proof =
    ...
    var result =
        Oberon.verifyProof(
            Security.VerifyOberonProofRequest.newBuilder()
                .setData(data)
                .setNonce(nonce)
                .setPk(key.getPk())
  ...
  void testVerifyToken() throws InvalidProtocolBufferException, DidException {
    ...
    var rightKey =
        Oberon.createKey(Security.CreateOberonKeyRequest.newBuilder().setSeed(rightSeed).build());
    var wrongKey =
        Oberon.createKey(Security.CreateOberonKeyRequest.newBuilder().setSeed(wrongSeed).build());
    ...
    var verifyRight =
        Oberon.verifyToken(
            Security.VerifyOberonTokenRequest.newBuilder()
                .setToken(token.getToken())
                .setData(data)
                .setPk(rightKey.getPk())
                .build());
    var verifyWrong =
        Oberon.verifyToken(
            Security.VerifyOberonTokenRequest.newBuilder()
