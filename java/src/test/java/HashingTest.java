...
public class HashingTest {
  ...
  private static final String keyedHash =
  ...
  private static final String deriveKey =
  ...
  void testBlake3KeyedHash() throws DidException, InvalidProtocolBufferException {
    var request =
        Hashing.Blake3KeyedHashRequest.newBuilder()
  ...
  void testBlake3DeriveKey() throws DidException, InvalidProtocolBufferException {
    var request =
        Hashing.Blake3DeriveKeyRequest.newBuilder()
            .setKeyMaterial(ByteString.copyFrom(data))
            .setContext(ByteString.copyFromUtf8(context))
            .build();
  ...
  void testSHA256() throws DidException, InvalidProtocolBufferException {
    var request =
        Hashing.SHA256HashRequest.newBuilder().setData(ByteString.copyFromUtf8("4113")).build();
    ...
    Assertions.assertEquals(
        ...
        Hex.bytesToHex(response.getDigest().toByteArray()));
  }
}
