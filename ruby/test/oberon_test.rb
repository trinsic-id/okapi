...
class OberonTest < Minitest::Test
  ...
  def test_oberon_demo
    key = Oberon.create_key(Security_V1::CreateOberonKeyRequest.new)
    data = 'alice'
    nonce = '1234'

    token = Oberon.create_token(Security_V1::CreateOberonTokenRequest.new(data: data, sk: key.sk))
    proof = Oberon.create_proof(Security_V1::CreateOberonProofRequest.new(data: data, nonce: nonce,
                                                                          token: token.token))
    result = Oberon.verify_proof(Security_V1::VerifyOberonProofRequest.new(data: data, nonce: nonce, pk: key.pk,
                                                                           proof: proof.proof))
