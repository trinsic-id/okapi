...
class OberonTest < Minitest::Test
  ...
  def test_oberon_demo
    ...
    proof = Okapi::Oberon.create_proof(Security_V1::CreateOberonProofRequest.new(data: data, nonce: nonce,
                                                                                 token: token.token))
    result = Okapi::Oberon.verify_proof(Security_V1::VerifyOberonProofRequest.new(data: data, nonce: nonce, pk: key.pk,
                                                                                  proof: proof.proof))
