# frozen_string_literal: true

require_relative 'test_helper'
require 'okapi/hashing/v1/hashing_pb'
require 'okapi'
require 'hashing'

class HashTest < Minitest::Test
  def before_setup
    Okapi.load_native_library
  end

  # rubocop:disable Layout/LineLength
  # taken from: https://raw.githubusercontent.com/BLAKE3-team/BLAKE3/master/test_vectors/test_vectors.json
  def test_blake3_hash
    hash = ''

    request = Hashing_V1::Blake3HashRequest.new(data: "\x00\x01\x02")
    response = Hashing.blake3_hash(request)

    assert !response.nil?
    assert !response.digest.nil?
    assert hash.start_with?(response.digest.unpack1('H*'))
  end

  def test_blake3_keyed_hash
    keyed_hash = ''
    request = Hashing_V1::Blake3KeyedHashRequest.new(data: "\x00\x01\x02", key: 'whats the Elvish word for friend')
    response = Hashing.blake3_keyed_hash(request)

    assert !response.nil?
    assert !response.digest.nil?
    assert keyed_hash.start_with?(response.digest.unpack1('H*'))
  end

  def test_blake3_derive_key
    derive_key = ''
    request = Hashing_V1::Blake3DeriveKeyRequest.new(key_material: "\x00\x01\x02", context: 'BLAKE3 2019-12-27 16:29:52 test vectors context')
    response = Hashing.blake3_derive_key(request)

    assert !response.nil?
    assert !response.digest.nil?
    assert derive_key.start_with?(response.digest.unpack1('H*'))
  end
  # rubocop:enable Layout/LineLength

  def test_sha256_hash
    hash = '71b3af35d9d53d24e7462177da41b8acd5e2ef4afc333dd9272cb2ab8743b3db'

    request = Hashing_V1::SHA256HashRequest.new(data: '4113')
    response = Hashing.sha256_hash(request)

    assert !response.nil?
    assert !response.digest.nil?
    assert_equal(hash, response.digest.unpack1('H*'))
  end
end
